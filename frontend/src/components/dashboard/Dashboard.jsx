...
import AmbassadorCommissionView from '../commissions/AmbassadorCommissionView';
...
const Dashboard = () => {
  ...
  const processDataForDashboard = (indications, commissions, users) => {
    if (user?.role === 'admin') {
      ...
      const activeAmbassadors = users.filter(u => u.role === 'embaixadora' && u.status === 'ativo').length;
      ...
      const userIndications = indications;
  ...
  const handleModalSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log('Criando nova indicação:', formData);
      ...
      if (response.status === 201) {
        console.log('Indicação criada com sucesso:', response.data);
        ...
        console.error('Erro ao criar indicação:', response.statusText);
  ...
  const AdminDashboardContent = () => {
    ...
    const leadsByOrigin = dashboardData?.charts?.leadsOrigin?.map(item => ({
      origem: getOriginDisplayName(item.name),
      leads: item.value
    })) || [];

    const conversionData = dashboardData?.charts?.conversionBySegment?.map(item => ({
      name: getSegmentDisplayName(item.segment),
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="card-green">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">COMISSÕES DO MÊS</p>
                <p className="text-white text-2xl font-bold mt-1">{formatCurrency(dashboardData?.stats?.monthlyCommissions || 0)}</p>
              </div>
          ...
          <div className="card-yellow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">CONVERSÃO INDICAÇÕES</p>
                <p className="text-2xl font-bold text-white mt-1">{(dashboardData?.stats?.activePercentage || 0).toFixed(2)}%</p>
              </div>
          ...
          <div className="card-red">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">TOTAL DE INDICAÇÕES</p>
                <p className="text-2xl font-bold text-white mt-1">{dashboardData?.stats?.totalIndications || 0}</p>
              </div>
          ...
          <div className="card-purple">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">EMBAIXADORAS ATIVAS</p>
                <p className="text-2xl font-bold text-white mt-1">{dashboardData?.stats?.activeAmbassadors || 0}</p>
              </div>
  ...
  const AmbassadorDashboardContent = () => {
    ...
    return (
      <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="card-green">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">TOTAL DE INDICAÇÕES</p>
                <p className="text-white text-2xl font-bold mt-1">{dashboardData?.totalIndications || 0}</p>
                <p className="text-white/70 text-sm mt-2">+{(dashboardData?.conversionRate || 0).toFixed(2)}% de conversão</p>
              </div>
              <Target className="h-8 w-8 text-white/80" />
            </div>
          </div>
          ...
          <div className="card-yellow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white text-sm font-medium">TOTAL DE VENDAS</p>
                <p className="text-white text-2xl font-bold mt-1">{dashboardData?.totalSales || 0}</p>
                <p className="text-white/70 text-sm mt-2">+13% acima da média</p>
              </div>
              <TrendingUp className="h-8 w-8 text-white/80" />
            </div>
          </div>
          ...
          <div className="card-purple">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white text-2xl font-bold mt-1">{formatCurrency(dashboardData?.monthlyCommission || 0)}</p>
        ...
        {/* Seção de Comissões Parceladas */}
        <div className="mt-8">
          <AmbassadorCommissionView />
        </div>
      </div>
    );
  };
